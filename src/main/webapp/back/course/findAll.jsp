...
<script>
    $(function () {
        $("#CourseDg").datagrid({
            ...
            url:"${pageContext.request.contextPath}/course/findPage",//发送请求
            height:630,
        ...
        if (rows.length<=0){
            ...
            $.ajax({
                ...
                success:function (result) {
                    if(result.success){
                        //提示消息
                        $.messager.show({title:'提示',msg:"删除多行成功"});
                    }else{
                        $.messager.show({title:'提示',msg:"删除多行失败"});
                    }
                    $("#CourseDg").datagrid('reload');//刷新当前datagrid
                },
            })
    ...
    function openSaveCourseDialog() {
        $("#saveCourseDialog").dialog({
            href:'${pageContext.request.contextPath}/back/course/save.jsp',
            title:'添加必修功课',
            buttons:[{
                iconCls:'icon-save',
                text:'保存',
                    ...
                    $("#saveCourseInputForm").form('submit',{
                        ...
                        success:function(result){//响应的一定是json格式字符串   使用应该先转为js对象
                            ...
                            if(resultObj.success){
                                ...
                                //提示信息
                                $.messager.show({title:'提示',msg:"添加用户成功"});
                            }
                            //关闭对话框
